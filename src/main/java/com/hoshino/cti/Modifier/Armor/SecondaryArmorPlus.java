        ...
        if (living instanceof Player player){
            living.getCapability(TinkerDataCapability.CAPABILITY).ifPresent((cap)->{
                ...
                if (level>0){
                    ...
                    event.setAmount(CombatRules.getDamageAfterAbsorb(event.getAmount(), (float) player.getArmorValue()*0.75f, (float) player.getAttributeValue(Attributes.ARMOR_TOUGHNESS)*0.75f));
                }
            });
        }
    }
