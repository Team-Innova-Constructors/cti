package com.hoshino.cti.Items.Vehicle;
...
public class RocketItemTier5<T extends Rocket> extends VehicleItem {
    private final int tier;

    public RocketItemTier5(EntityType<T> rocketEntity, int tier, Properties properties) {
        super(properties);
        this.tier = tier;
    }
        ...
        if (!level.isClientSide) {
            ...
            if (state.getBlock() instanceof LaunchPad) {
                if (state.getValue(LaunchPad.LOCATION).equals(LocationState.CENTER)) {
                    ...
                    if (rocketStack.getStack().getItem() instanceof RocketItemTier5<?> rocket) {
